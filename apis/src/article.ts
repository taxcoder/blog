...
export const useArticle = (base: string = 'api') => {
  ...
  return { articleList, archivedList, findArticleContentById, findArticleInfoById, articleClassification, articleTag };
};
...
export const useAuthArticle = (base: string = 'auth') => {
  ...
  const articleUpdateById = async (
    ...
    return await service.put(`/${base}/article/update/info/${id}`, obj);
  };
  ...
  const addArticle = async (obj: {
    ...
    return await service.post(`/${base}/article/add/info`, obj);
  };
