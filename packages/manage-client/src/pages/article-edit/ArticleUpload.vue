...
const submit = () => {
  ...
  if (!classificationId) {
    ...
    let img = formState.image;
    // 如果没有图片，后面自己添加，如果是本地上传的图片，则没有url
    let image = img.length > 0 ? (img[0].url ? img[0].url : img[0].thumbUrl) : null;
    auth
...
const beforeUpload: any = (file: any) => {
  formState.image = [...(formState.image || []), file];
  return false;
};
...
watch(
  ...
  (state: any) => {
    ...
    console.log(state, tagValues.value);
  },
