...
const item = (r: any, prefix = '') => ({
  ...
  icon: r.icon(),
  url: prefix + r.url,
  children: r.children.length === 0 ? undefined : r.children.map((c: any) => item(c, r.url)),
});

const items = computed(() =>
  routerMenu
    .map((r: any) => {
      if (r.key === 'refresh') return false;
      return item(r);
    })
...
watchEffect(() => {
  ...
  state.selectedKeys = [node.children.find((i: any) => i.label === route.name).key];
});
</script>
