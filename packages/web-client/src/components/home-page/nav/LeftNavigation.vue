<template>
  ...
  >
    <template #prefix>
      <span
        class="title sm:text-[25px] text-[1.2rem] cursor-pointer text-white mr-[10px] ml-[20px] transition-font-size"
        :class="{ 'important-text-[#000000]': props.isActive }"
...
import { useCommon } from '@tanxiang/utils/index';
...
const updateRouterBarPosition = (index: number) => {
  let wrapper = document.querySelector('.router-tab-bar');
...
// 判断是否是电脑
const juiceIsComputer = computed(() => status.width > minWidth);
