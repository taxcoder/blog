...
// 获取已运行时间
const getRunTime = computed(() => days(global.getWebSite.createTime));
// 获取上次登录时间
const getLastUpdateTime = computed(() => {
  return !juiceIsNotActive ? headway(global.getWebSite.loginUpdateTime + 1000 * 60 * 60, true) + '前' : '当前在线';
});
// 获取网站总访问量
...
// 判断登录时间是否超过了一小时
const juiceIsNotActive = computed(() => new Date().getTime() - global.getWebSite.loginUpdateTime <= 1000 * 60 * 60);
