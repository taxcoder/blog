...
onMounted(() => {
  ...
  tag
    .tagArticle(current.value, size.value, route.params.id)
    .then((success: any) => {
      if (success.total === 0) return router.push('/');
      loading.value = false;
      articleList.value = success;
      success.records[0].tag.forEach((record: any) => {
...
const queryNextData = () => {
  tag
    .tagArticle(current.value + 1, size.value, route.params.id)
    .then((result: any) => {
...
const sizeChange = (obj: { current: number; size: number }) => {
  tag
    .tagArticle(obj.current, obj.size, route.params.id)
    .then((success: any) => {
      size.value = obj.size;
